...
- 可解释话题聚合、四种文章模式、编辑参数与来源可追溯的本地模板生成。
...
阶段四默认使用 `DRAFT_GENERATOR_PROVIDER=template` 的本地证据模板，不需要模型 API Key。它只读取当前话题关联的来源，并在 Markdown 末尾保留链接；没有足够证据时会明确说明信息不足。后续可在不改变页面或数据库结构的前提下替换 `DraftGenerator` 实现接入模型服务。
